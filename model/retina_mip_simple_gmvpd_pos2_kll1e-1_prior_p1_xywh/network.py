...
from det_oprs.loss_opr import mip_loss_focal, mip_pos2_gmvpd_loss_focal
...
class RetinaNet_Criteria(nn.Module):
    ...
    def __call__(self, pred_cls_list, pred_reg_list, anchors_list, gt_boxes, im_info):
        ...
        all_pred_reg = torch.cat(pred_reg_list, axis=1).reshape(-1, 8*2)
        # get ground truth
        ...
        all_pred_reg = all_pred_reg.reshape(-1, 2, 8)
        loss = mip_loss_focal(
                all_pred_reg[:, 0, :4], all_pred_cls[:, 0],
                all_pred_reg[:, 1, :4], all_pred_cls[:, 1],
                bbox_targets, labels)
        loss1 = mip_pos2_gmvpd_loss_focal(
...
class RetinaNet_Head(nn.Module):
    def __init__(self):
        ...
        self.bbox_pred = nn.Conv2d(
            in_channels, config.num_cell_anchors * 8 * 2,
            kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, features):
        ...
        pred_cls_list = [
            ...
            for _ in pred_cls]
        pred_reg_list = [
            _.permute(0, 2, 3, 1).reshape(pred_reg[0].shape[0], -1, 8*2)
            for _ in pred_reg]
...
def per_layer_inference(anchors_list, pred_cls_list, pred_reg_list, im_info):
    ...
    for l_id in range(len(anchors_list)):
        ...
        pred_reg = pred_reg_list[l_id][0].reshape(-1, 8*2)
    ...
    pred_delta_0 = keep_reg.reshape(-1,2,8)[:, 0, :4]
    pred_delta_1 = keep_reg.reshape(-1,2,8)[:, 1, 4:]
