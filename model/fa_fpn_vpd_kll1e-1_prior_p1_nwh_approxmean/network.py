...
from det_oprs.my_loss_opr import freeanchor_avpd_loss
...
class Network(nn.Module):
    def __init__(self):
        ...
        self.RA_Anchor = RetinaNet_Approx_Anchor()
    ...
    def forward(self, image, im_info, epoch=None, gt_boxes=None):
        ...
        approx_anchors_list = self.RA_Anchor(fpn_fms)
        pred_cls_list, pred_reg_list = self.R_Head(fpn_fms)
        # release the useless data
        if self.training:
            loss_dict = self.R_Criteria(
                    pred_cls_list, pred_reg_list, anchors_list, approx_anchors_list, gt_boxes, im_info)
            return loss_dict
        else:
...
class RetinaNet_Approx_Anchor():
    def __init__(self):
        self.anchors_generator = AnchorGenerator(
            config.anchor_base_size,
            config.approx_anchor_aspect_ratios,
            config.approx_anchor_base_scale)
...
class RetinaNet_Criteria(nn.Module):
    ...
    def __call__(self, pred_cls_list, pred_reg_list, anchors_list, approx_anchors_list, gt_boxes, im_info):
        all_anchors = torch.cat(anchors_list, axis=0)
        approx_anchors_list = [anchor.reshape(-1,config.num_cell_approx_anchors,4) 
                                for anchor in approx_anchors_list]
        all_approx_anchors = torch.cat(approx_anchors_list, axis=0)
        ...
        approx_samples = bbox_transform_opr(all_anchors.unsqueeze(1).repeat(
                                    1,config.num_cell_approx_anchors,1).reshape(
                                    -1,4), all_approx_anchors.reshape(-1,4))
        all_pred_reg = all_pred_mean

        # get ground truth
        loss_dict = freeanchor_avpd_loss(all_anchors, approx_samples, all_pred_cls, 
                                        all_pred_reg, pred_scale_std, gt_boxes, im_info)
