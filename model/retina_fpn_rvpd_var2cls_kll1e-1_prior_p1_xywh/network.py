...
class Network(nn.Module):
    ...
    def forward(self, image, im_info, epoch=None, gt_boxes=None):
        ...
        pred_cls_list, pred_refined_cls_list, pred_reg_list = self.R_Head(fpn_fms)
        # release the useless data
        if self.training:
            loss_dict = self.R_Criteria(
                    pred_cls_list, pred_refined_cls_list, pred_reg_list, anchors_list, gt_boxes, im_info)
            return loss_dict
        else:
...
class RetinaNet_Criteria(nn.Module):
    ...
    def __call__(self, pred_cls_list, pred_refined_cls_list, pred_reg_list, anchors_list, gt_boxes, im_info):
...
class RetinaNet_Head(nn.Module):
    def __init__(self):
        ...
        for refined_cls in self.refined_cls_pred:
            torch.nn.init.constant_(refined_cls.bias, bias_value)
    ...
    def forward(self, features):
        ...
        pred_reg_list = [
            ...
            for _ in pred_reg]
        return pred_cls_list, pred_refined_cls_list, pred_reg_list
