...
from det_oprs.loss_opr import mip_loss_softmax, mip_loss_softmax_reg
...
class RCNN(nn.Module):
    def __init__(self):
        ...
        self.pred_delta_0 = nn.Linear(1024, config.num_classes * 4)
        self.pred_cls_1 = nn.Linear(1024, config.num_classes)
        self.pred_delta_1 = nn.Linear(1024, config.num_classes * 4)
        ...
        for l in [self.pred_cls_0, self.pred_cls_1]:
            ...
            nn.init.constant_(l.bias, 0)
        for l in [self.pred_delta_0, self.pred_delta_1,
    ...
    def forward(self, fpn_fms, rcnn_rois, labels=None, bbox_targets=None):
        ...
        pred_scores_0 = F.softmax(pred_cls_0, dim=-1)
        pred_scores_1 = F.softmax(pred_cls_1, dim=-1)
        # cons refine feature
        boxes_feature_0 = torch.cat((pred_delta_0[:, 4:],
            pred_scores_0[:, 1][:, None]), dim=1).repeat(1, 4)
        boxes_feature_1 = torch.cat((pred_delta_1[:, 4:],
            pred_scores_1[:, 1][:, None]), dim=1).repeat(1, 4)
        ...
        if self.training:
            loss = mip_loss_softmax(
                        ...
                        bbox_targets, labels)
            loss1 = mip_loss_softmax_reg(
                        pred_ref_delta_0, pred_cls_0,
                        pred_ref_delta_1, pred_cls_1,
                        bbox_targets, labels)
            # requires_grad = False
            loss_mip = loss.mean()
            loss_ref = loss1.mean()
