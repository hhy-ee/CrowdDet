...
class RetinaNet_Criteria(nn.Module):
    ...
    def __call__(self, pred_cls_list, pred_reg_list, anchors_list, gt_boxes, im_info):
        ...
        proj_mean = torch.tensor(config.component).type_as(all_pred_dist).\
            repeat(weight.shape[0], 1)
        gaus_dist = all_pred_dist[..., n_component:]
        pred_mean_offset = torch.tanh(gaus_dist[..., :n_component]).reshape(-1, n_component)
        pred_mean = pred_mean_offset * config.component[0, -1] + proj_mean
