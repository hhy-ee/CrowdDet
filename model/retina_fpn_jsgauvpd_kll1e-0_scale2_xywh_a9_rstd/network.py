...
class RetinaNet_Head(nn.Module):
    ...
    def forward(self, features):
        ...
        for feature in features:
            ...
            # refinement
            if config.stat_mode == 'std':
                N, _, W, H = bbox_pred.shape
                stat = bbox_pred[:, 4 * config.num_cell_anchors:]
            quality_score = self.reg_conf(stat.reshape(-1, 4, W, H))
            quality_score = quality_score.reshape(N, -1, W, H)
