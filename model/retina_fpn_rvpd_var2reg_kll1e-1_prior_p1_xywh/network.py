    ...
    def forward(self, image, im_info, epoch=None, gt_boxes=None):
        ...
        if self.training:
            ...
            pred_bbox = per_layer_inference(
                    anchors_list, pred_reg_list, pred_refined_reg_list, im_info)
...
class RetinaNet_Criteria(nn.Module):
    ...
    def __call__(self, pred_cls_list, pred_reg_list, pred_refined_reg_list, anchors_list, gt_boxes, im_info):
        ...
        all_pred_refined_reg = torch.cat(pred_refined_reg_list, axis=1).reshape(-1, 4)
        ...
        loss_refined_reg = smooth_l1_loss(
                all_pred_refined_reg[fg_mask],
                bbox_target[fg_mask],
                config.smooth_l1_beta)      
        loss_cls = focal_loss(
