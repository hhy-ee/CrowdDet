...
class Network(nn.Module):
    ...
    def forward(self, image, im_info, epoch=None, gt_boxes=None):
        ...
        pred_cls_list, pred_ref_cls_list, pred_reg_list, pred_ref_reg_list = self.R_Head(fpn_fms)
        # release the useless data
        if self.training:
            loss_dict = self.R_Criteria(
                pred_cls_list, pred_ref_cls_list, pred_reg_list, 
                pred_ref_reg_list, anchors_list, gt_boxes, im_info)
            return loss_dict
        else:
            #pred_bbox = union_inference(
            #        anchors_list, pred_cls_list, pred_reg_list, im_info)
            pred_bbox = per_layer_inference(
                    anchors_list, pred_ref_cls_list, pred_ref_reg_list, im_info)
...
class RetinaNet_Criteria(nn.Module):
    ...
    def __call__(self, pred_cls_list, pred_ref_cls_list, pred_reg_list,
            pred_ref_reg_list, anchors_list, gt_boxes, im_info):
        ...
        all_pred_ref_cls = torch.cat(pred_ref_cls_list, axis=1).reshape(-1, (config.num_classes-1)*2)
        all_pred_ref_cls = torch.sigmoid(all_pred_ref_cls)
        all_pred_reg = torch.cat(pred_reg_list, axis=1).reshape(-1, 4*2)
        all_pred_ref_reg = torch.cat(pred_ref_reg_list, axis=1).reshape(-1, 4*2)
        # get ground truth
        ...
        all_pred_ref_cls = all_pred_ref_cls.reshape(-1, 2, config.num_classes-1)
        all_pred_ref_reg = all_pred_ref_reg.reshape(-1, 2, 4)
        ...
        loss1 = emd_loss_focal(
                ...
                all_pred_reg[:, 0], all_pred_cls[:, 0],
                bbox_targets, labels)
        loss2 = emd_loss_focal(
                all_pred_ref_reg[:, 0], all_pred_ref_cls[:, 0],
                all_pred_ref_reg[:, 1], all_pred_ref_cls[:, 1],
                bbox_targets, labels)
        loss3 = emd_loss_focal(
                all_pred_ref_reg[:, 1], all_pred_ref_cls[:, 1],
                all_pred_ref_reg[:, 0], all_pred_ref_cls[:, 0],
                bbox_targets, labels)
        del all_anchors
        ...
        _, min_ref_indices = loss_ref.min(axis=1)
        loss_emd = loss[torch.arange(loss.shape[0]), min_indices]
        loss_emd_ref = loss_ref[torch.arange(loss_ref.shape[0]), min_ref_indices]
        # only main labels
        ...
        loss_emd_ref = loss_emd_ref.sum() / self.loss_normalizer
        loss_dict = {}
        loss_dict['retina_emd'] = loss_emd
        loss_dict['retina_emd_ref'] = loss_emd_ref
        return loss_dict
...
class RetinaNet_Head(nn.Module):
    ...
    def forward(self, features):
        ...
        pred_ref_cls_list = [
            _.permute(0, 2, 3, 1).reshape(pred_cls[0].shape[0], -1, (config.num_classes-1)*2)
            for _ in pred_ref_cls]
        pred_reg_list = [
            _.permute(0, 2, 3, 1).reshape(pred_reg[0].shape[0], -1, 4*2)
            for _ in pred_reg]
        pred_ref_reg_list = [
            _.permute(0, 2, 3, 1).reshape(pred_reg[0].shape[0], -1, 4*2)
            for _ in pred_ref_reg]
        return pred_cls_list, pred_ref_cls_list, pred_reg_list, pred_ref_reg_list
