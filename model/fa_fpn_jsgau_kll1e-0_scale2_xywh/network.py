...
class Network(nn.Module):
    ...
    def forward(self, image, im_info, epoch=None, gt_boxes=None, epoch_id=None, id=None):
        ...
        if self.training:
            ...
            if config.save_data:
                per_layer_savebbox(
                        anchors_list, pred_cls_list, pred_reg_list, gt_boxes, im_info, epoch_id, id)
...
def per_layer_savekeep(anchors_list, pred_cls_list, pred_reg_list, gt_boxes, im_info, epoch_id, id):
    keep_anchors = []
    keep_cls = []
    keep_reg = []
    keep_lstd = []
    ...
    vis_keep = torch.where((normalize_target[:, 0] > -100))
    save_data['ID'] = id[0]
...
def per_layer_savebbox(anchors_list, pred_cls_list, pred_reg_list, gt_boxes, im_info, epoch_id, id):
    ...
    if gts_keep.shape[0] != 0:
        ...
        normalize_bbox = target_normalize(keep_bboxes, keep_gtbox, 'xy')
        f = open("./nmbox/normalize_bbox_{}.txt".format(str(epoch_id)),'a')
        data = normalize_bbox.detach().cpu().numpy()
        np.savetxt(f, data)
        f.close()
        if epoch_id == 30:
            keep_bboxes = anchors[vis_keep]
            keep_gtbox = torch.tensor(gts_keep).type_as(keep_bboxes)
            normalize_bbox = target_normalize(keep_bboxes, keep_gtbox, 'xy')
            f = open("./nmbox/normalize_bbox_0.txt",'a')
            data = normalize_bbox.detach().cpu().numpy()
            np.savetxt(f, data)
            f.close()
