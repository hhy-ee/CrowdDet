...
from det_oprs.loss_opr import js_gmm_loss
...
class RetinaNet_Criteria(nn.Module):
    ...
    def __call__(self, pred_cls_list, pred_reg_list, anchors_list, gt_boxes, im_info):
        ...
        gaus_dist = all_pred_dist[..., n_component:]
        pred_mean = weight.new_full(weight.shape, config.component[0, 0])
        pred_quan = torch.softmax(gaus_dist[..., :n_component - 1], dim=2)
        pred_quan = torch.cumsum(pred_quan, dim=2).reshape(-1, n_component-1)
        pred_mean[:, 1:] =  scale * pred_quan + config.project[0, 0]
        pred_lstd = gaus_dist[..., n_component:].reshape(-1, n_component)
        pred_gumbel_reg = pred_mean + pred_lstd.exp() * torch.randn_like(pred_mean)
        pred_delta = weight.mul(pred_mean).sum(dim=1).reshape(-1, 4)
        pred_gumbel_delta = gumbel_weight.mul(pred_gumbel_reg).sum(dim=1).reshape(-1, 4)
        # freeanchor loss
        loss_dict = freeanchor_vpd_loss_sml(
            all_anchors, all_pred_cls, pred_delta, pred_gumbel_delta, gt_boxes, im_info)
        # js loss
        ...
        pos_pred_mean = pred_mean.reshape(-1, 4, n_component)[fg_mask].reshape(-1, n_component)
        ...
        loss_dict['freeanchor_jsd_loss'] = loss_jsd
        return loss_dict
