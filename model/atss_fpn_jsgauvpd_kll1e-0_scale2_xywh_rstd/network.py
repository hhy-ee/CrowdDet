...
class Network(nn.Module):
    ...
    def forward(self, image, im_info, epoch=None, gt_boxes=None, id=None):
        ...
        pred_cls_list, pred_reg_list, pred_ctn_list, pred_refined_cls_list = self.R_Head(fpn_fms)
        # release the useless data
        if self.training:
            loss_dict = self.R_Criteria(
                    pred_cls_list, pred_reg_list, pred_ctn_list, pred_refined_cls_list, anchors_list, gt_boxes, im_info)
            return loss_dict
        else:
            #pred_bbox = union_inference(
            #        anchors_list, pred_cls_list, pred_reg_list, im_info)
            pred_bbox = per_layer_inference(
                    anchors_list, pred_cls_list, pred_reg_list, pred_ctn_list, pred_refined_cls_list, im_info)
...
class RetinaNet_Criteria(nn.Module):
    ...
    def __call__(self, pred_cls_list, pred_reg_list, pred_ctn_list, pred_refined_cls_list, anchors_list, gt_boxes, im_info):
        ...
        loss_refined_cls = focal_loss(
                ...
                config.focal_loss_alpha,
                config.focal_loss_gamma)
        loss_cls = (loss_cls + loss_refined_cls) / 2
