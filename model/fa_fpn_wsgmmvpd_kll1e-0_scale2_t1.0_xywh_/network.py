...
from det_oprs.loss_opr import ws_gmm_loss_
...
class RetinaNet_Criteria(nn.Module):
    ...
    def __call__(self, pred_cls_list, pred_reg_list, anchors_list, gt_boxes, im_info):
        ...
        pred_gauss_delta = project_mean + pred_lstd.exp() * torch.randn_like(project_mean)
        pred_gumbel_delta = gumbel_weight.mul(pred_gauss_delta).sum(dim=1).reshape(-1, 4)
        ...
        loss_dict['freeanchor_wsd_loss'] = loss_wsd
        return loss_dict
