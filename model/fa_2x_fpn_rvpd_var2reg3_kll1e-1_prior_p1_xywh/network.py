...
class Network(nn.Module):
    ...
    def forward(self, image, im_info, epoch=None, gt_boxes=None):
        ...
        pred_cls_list, pred_reg_list, features, refine_subset, ref_cls, ref_reg = self.R_Head(fpn_fms)
        # release the useless data
        if self.training:
            loss_dict = self.R_Criteria(
                pred_cls_list, pred_reg_list, features, 
                refine_subset, ref_cls, ref_reg, anchors_list, gt_boxes, im_info)
            return loss_dict
        else:
...
class RetinaNet_Criteria(nn.Module):
    ...
    def __call__(self, pred_cls_list, pred_reg_list, features, 
            refine_subset, ref_cls, ref_reg, anchors_list, gt_boxes, im_info):
        # refine feature
        pred_ref_cls_list = []
        pred_ref_reg_list = []
        pred_va_reg_list = []
        for (feature, pred_cls, pred_reg) in zip(features, pred_cls_list, pred_reg_list):
            # variational inference
            pred_mean = pred_reg[:, :config.num_cell_anchors * 4]
            pred_lstd = pred_reg[:, config.num_cell_anchors * 4:]
            scale = torch.tensor(config.prior_std).type_as(pred_mean)
            pred_scale_std = pred_lstd.exp().permute(0,2,3,1).mul(scale).permute(0,3,1,2)
            pred_va_reg = pred_mean + pred_scale_std * torch.randn_like(pred_lstd)
            boxes_feature = torch.cat((pred_cls, pred_va_reg), dim=1)
            boxes_feature = torch.cat((feature, boxes_feature), dim=1)
            refine_feature = refine_subset(boxes_feature)
            pred_ref_cls_list.append(ref_cls(refine_feature))
            pred_ref_reg_list.append(ref_reg(refine_feature))
            pred_va_reg_list.append(pred_va_reg)
        # reshape the predictions
        assert pred_cls_list[0].dim() == 4
        pred_cls_list = [
            _.permute(0, 2, 3, 1).reshape(pred_cls_list[0].shape[0], -1, config.num_classes-1)
            for _ in pred_cls_list]
        pred_ref_cls_list = [
            _.permute(0, 2, 3, 1).reshape(pred_cls_list[0].shape[0], -1, config.num_classes-1)
            for _ in pred_ref_cls_list]
        pred_reg_list = [
            _.permute(0, 2, 3, 1).reshape(pred_cls_list[0].shape[0], -1, 8)
            for _ in pred_reg_list]
        pred_va_reg_list = [
            _.permute(0, 2, 3, 1).reshape(pred_cls_list[0].shape[0], -1, 4)
            for _ in pred_va_reg_list]
        pred_ref_reg_list = [
            _.permute(0, 2, 3, 1).reshape(pred_cls_list[0].shape[0], -1, 8)
            for _ in pred_ref_reg_list]
        ...
        all_pred_ref_cls = torch.cat(pred_ref_cls_list, axis=1).reshape(-1, config.num_classes-1)
        all_pred_ref_cls = torch.sigmoid(all_pred_ref_cls)
        all_pred_reg = torch.cat(pred_reg_list, axis=1).reshape(-1, 8)
        all_pred_va_reg = torch.cat(pred_va_reg_list, axis=1).reshape(-1, 4)
        all_pred_ref_reg = torch.cat(pred_ref_reg_list, axis=1).reshape(-1, 8)[:, :4]
        ...
        # get ground truth
        loss_dict = freeanchor_loss(all_anchors, all_pred_cls, all_pred_va_reg, gt_boxes, im_info)
...
class RetinaNet_Head(nn.Module):
    def __init__(self):
        ...
        for _ in range(num_convs):
            ...
            if _ == 0:
                refine_subset.append(
                    nn.Conv2d(in_ref_channel, in_channels, kernel_size=3, stride=1, padding=1)
                )
            else:
                refine_subset.append(
                    nn.Conv2d(in_channels, in_channels, kernel_size=3, stride=1, padding=1)
                )
            refine_subset.append(nn.ReLU())
        self.cls_subnet = nn.Sequential(*cls_subnet)
        self.bbox_subnet = nn.Sequential(*bbox_subnet)
        self.refine_subset = nn.Sequential(*refine_subset)
        # predictor
        ...
        self.bbox_pred = nn.Conv2d(
            ...
            kernel_size=3, stride=1, padding=1)
        self.ref_cls_score = nn.Conv2d(
            in_channels, config.num_cell_anchors * (config.num_classes-1),
            kernel_size=3, stride=1, padding=1)
        self.ref_bbox_pred = nn.Conv2d(
            in_channels, config.num_cell_anchors * 8,
            kernel_size=3, stride=1, padding=1)

        # Initialization
        for modules in [self.cls_subnet, self.bbox_subnet, self.refine_subset,
                        self.cls_score,  self.bbox_pred, self.ref_cls_score, self.ref_bbox_pred]:
        ...
        torch.nn.init.constant_(self.ref_cls_score.bias, bias_value)
