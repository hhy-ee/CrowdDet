...
from det_oprs.loss_opr import softmax_loss, refined_var_softmax_loss, smooth_l1_loss, rcnn_kldiv_loss
...
class RCNN(nn.Module):
    ...
    def forward(self, fpn_fms, rcnn_rois, labels=None, bbox_targets=None):
        ...
        pred_var_cls = self.pred_var_cls(in_cls)
        ...
        if self.training:
            ...
            objectness_loss, var_objectness_loss, refined_objectness_loss = refined_var_softmax_loss(
                pred_cls, pred_var_cls, pred_weight, labels)
            ...
            class_num = pred_refined_cls.shape[-1] - 1
            tag = torch.arange(class_num).type_as(pred_refined_cls)+1
            tag = tag.repeat(pred_refined_cls.shape[0], 1).reshape(-1,1)
            pred_scores = F.softmax(pred_refined_cls, dim=-1)[:, 1:].reshape(-1, 1)
