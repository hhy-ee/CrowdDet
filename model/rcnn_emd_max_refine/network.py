...
class RCNN(nn.Module):
    ...
    def forward(self, fpn_fms, rcnn_rois, labels=None, bbox_targets=None):
        ...
        if self.training:
            ...
            # requires_grad = False
            _, max_indices_rcnn = loss_rcnn.max(axis=1)
            _, max_indices_ref = loss_ref.max(axis=1)
            loss_rcnn = loss_rcnn[torch.arange(loss_rcnn.shape[0]), max_indices_rcnn]
            loss_rcnn = loss_rcnn.mean()
            loss_ref = loss_ref[torch.arange(loss_ref.shape[0]), max_indices_ref]
