...
class RetinaNet_Criteria(nn.Module):
    ...
    def __call__(self, pred_cls_list, pred_reg_list, pred_ctn_list, anchors_list, gt_boxes, im_info):
        ...
        project_mean = torch.tensor(config.project).type_as(all_pred_dist).repeat(pos_weight.shape[0], 1)
        pos_pred_lstd = all_pred_dist[..., n_component:][fg_mask].reshape(-1, n_component)
        pos_pred_delta = project_mean
        pos_pred_delta = pos_weight.mul(pos_pred_delta).sum(dim=1).reshape(-1, 4)
        # regression loss
