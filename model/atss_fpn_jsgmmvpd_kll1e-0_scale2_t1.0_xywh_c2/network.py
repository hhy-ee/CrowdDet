...
class RetinaNet_Criteria(nn.Module):
    ...
    def __call__(self, pred_cls_list, pred_reg_list, pred_ctn_list, anchors_list, gt_boxes, im_info):
        ...
        all_pred_dist = torch.cat(pred_reg_list, axis=1).reshape(-1, 4, 2 * config.component.shape[1])
        # get ground truth
        ...
        pos_proj_mean = torch.tensor(config.component).type_as(all_pred_dist).repeat(pos_weight.shape[0], 1)
        pos_pred_lstd = all_pred_dist[..., n_component:][fg_mask].reshape(-1, n_component)
        pos_pred_delta = pos_proj_mean + pos_pred_lstd.exp() * torch.randn_like(pos_proj_mean)
...
class RetinaNet_Head(nn.Module):
    def __init__(self):
        ...
        self.bbox_pred = nn.Conv2d(
            in_channels, config.num_cell_anchors * 4 * 2 * config.component.shape[1],
            kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, features):
        ...
        pred_cls_list = [
            ...
            for _ in pred_cls]
        pred_reg_list = [
            _.permute(0, 2, 3, 1).reshape(pred_reg[0].shape[0], -1, 4 * 2 * config.component.shape[1])
            for _ in pred_reg]
        pred_ctn_list = [
...
def per_layer_inference(anchors_list, pred_cls_list, pred_reg_list, pred_ctn_list, im_info):
    ...
    for l_id in range(len(anchors_list)):
        ...
        pred_reg = pred_reg_list[l_id][0].reshape(-1, 2 * config.component.shape[1])
        ...
        pred_reg = pred_wgh.mul(proj_mod).sum(dim=1).reshape(-1, 4)
