...
from det_oprs.loss_opr import focal_loss, smooth_l1_loss, entropy_loss
...
class RetinaNet_Criteria(nn.Module):
    ...
    def __call__(self, pred_cls_list, pred_reg_list, anchors_list, gt_boxes, im_info):
        ...
        loss_cat = torch.abs(entropy_loss(all_pred_reg[fg_mask]).mean(dim=1) - 2.30258) * config.kl_weight
