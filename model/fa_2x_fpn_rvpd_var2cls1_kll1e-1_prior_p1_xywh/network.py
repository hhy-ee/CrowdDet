...
class Network(nn.Module):
    ...
    def forward(self, image, im_info, epoch=None, gt_boxes=None):
        ...
        pred_cls_list, pred_var_cls_list, pred_refined_cls_list, pred_reg_list = self.R_Head(fpn_fms)
        # release the useless data
        if self.training:
            loss_dict = self.R_Criteria(
                pred_cls_list, pred_var_cls_list, pred_refined_cls_list, 
                pred_reg_list, anchors_list, gt_boxes, im_info)
            return loss_dict
        else:
...
class RetinaNet_Criteria(nn.Module):
    ...
    def __call__(self, pred_cls_list, pred_var_cls_list, pred_refined_cls_list, 
            pred_reg_list, anchors_list, gt_boxes, im_info):
        ...
        all_pred_var_cls = torch.cat(pred_var_cls_list, axis=1).reshape(-1, config.num_classes-1)
        all_pred_var_cls = torch.sigmoid(all_pred_var_cls)
        ...
        var_loss_dict = freeanchor_loss(all_anchors, all_pred_var_cls, all_pred_reg, gt_boxes, im_info)
...
class RetinaNet_Head(nn.Module):
    def __init__(self):
        ...
        self.bbox_pred = nn.Conv2d(
            ...
            kernel_size=3, stride=1, padding=1)
        # refined reg predict
        self.var_cls_pred = nn.Sequential(
        ...
        for modules in [self.cls_subnet, self.bbox_subnet, self.cls_score, 
                        self.bbox_pred, self.var_cls_pred, self.refined_cls_pred]:
        ...
        torch.nn.init.constant_(self.var_cls_pred[-1].bias, bias_value)
    ...
    def forward(self, features):
        ...
        for (cls, reg) in zip(pred_cls, pred_reg):
            var_cls = self.var_cls_pred(reg[:, 4:])
            refined_cls = self.refined_cls_pred(torch.cat([cls, reg[:, 4:]], dim=1))
            pred_var_cls.append(var_cls)
        ...
        pred_reg_list = [
            ...
            for _ in pred_reg]
        return pred_cls_list, pred_var_cls_list, pred_refined_cls_list, pred_reg_list
