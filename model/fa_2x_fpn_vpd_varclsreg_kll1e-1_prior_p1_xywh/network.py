...
class Network(nn.Module):
    ...
    def forward(self, image, im_info, epoch=None, gt_boxes=None):
        ...
        pred_cls_list, pred_refined_cls_list, pred_reg_list, pred_refined_reg_list = self.R_Head(fpn_fms)
        # release the useless data
        if self.training:
            loss_dict = self.R_Criteria(
                pred_cls_list, pred_refined_cls_list, pred_reg_list, 
                pred_refined_reg_list, anchors_list, gt_boxes, im_info)
            return loss_dict
        else:
            #pred_bbox = union_inference(
            #        anchors_list, pred_cls_list, pred_reg_list, im_info)
            pred_bbox = per_layer_inference(
                    anchors_list, pred_refined_cls_list, pred_reg_list, pred_refined_reg_list, im_info)
...
class RetinaNet_Criteria(nn.Module):
    ...
    def __call__(self, pred_cls_list, pred_refined_cls_list, pred_reg_list, 
                pred_refined_reg_list, anchors_list, gt_boxes, im_info):
        ...
        refined_loss_dict = freeanchor_loss(all_anchors, all_pred_refined_cls, all_refined_reg_list, gt_boxes, im_info)
        loss_kld = kldiv_loss(
                all_pred_mean,
                all_pred_lstd,
                config.kl_weight)
...
class RetinaNet_Head(nn.Module):
    def __init__(self):
        ...
        for modules in [self.cls_subnet, self.bbox_subnet, self.cls_score, 
                        self.bbox_pred, self.refined_cls_pred, self.refined_reg_pred]:
    ...
    def forward(self, features):
        ...
        return pred_cls_list, pred_refined_cls_list, pred_reg_list, pred_refined_reg_list
