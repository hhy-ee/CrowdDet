...
from det_oprs.loss_opr import emd_loss_focal, mip_pos_gmvpd_loss_focal
...
class RetinaNet_Criteria(nn.Module):
    ...
    def __call__(self, pred_cls_list, pred_reg_list, anchors_list, gt_boxes, im_info):
        ...
        loss1 = emd_loss_focal(
                ...
                all_pred_reg[:, 0, :4], all_pred_cls[:, 0],
                bbox_targets, labels)
        loss2 = mip_pos_gmvpd_loss_focal(
        ...
        loss_gm_loc = loss2['loss_loc'].sum() / self.loss_normalizer
        loss_gm_cat = loss2['loss_cat'].sum() / self.loss_normalizer
        loss_gm_kld = loss2['loss_kld'].sum() / self.loss_normalizer
        loss_dict = {}
