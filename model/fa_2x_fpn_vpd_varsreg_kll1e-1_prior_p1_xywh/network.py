...
class Network(nn.Module):
    ...
    def forward(self, image, im_info, epoch=None, gt_boxes=None):
        ...
        pred_cls_list, pred_reg_list, refined_net = self.R_Head(fpn_fms)
        # release the useless data
        if self.training:
            loss_dict = self.R_Criteria(
                pred_cls_list, pred_reg_list, refined_net, anchors_list, gt_boxes, im_info)
            return loss_dict
        else:
            #pred_bbox = union_inference(
            #        anchors_list, pred_cls_list, pred_reg_list, im_info)
            pred_bbox = per_layer_inference(
                    anchors_list, pred_cls_list, pred_reg_list, refined_net, im_info)
...
class RetinaNet_Criteria(nn.Module):
    ...
    def __call__(self, pred_cls_list, pred_reg_list, refined_net, anchors_list, gt_boxes, im_info):
        ...
        all_pred_reg = []
        all_refined_pred_reg = []
        all_pred_mean = []
        all_pred_lstd = []
        for pred_reg in pred_reg_list:
            pred_mean = pred_reg[:, :config.num_cell_anchors * 4]
            pred_lstd = pred_reg[:, config.num_cell_anchors * 4:]
            scale = torch.tensor(config.prior_std).type_as(pred_lstd)
            pred_scale_std = pred_lstd.exp().permute(0,2,3,1).mul(scale).permute(0,3,1,2)
            pred_reg = pred_mean + pred_scale_std * torch.randn_like(pred_lstd)
            # refined regression
            in_refined_reg = torch.cat([pred_reg, pred_lstd], dim=1)
            refined_pred_reg = refined_net(in_refined_reg)
            all_pred_reg.append(pred_reg.permute(0, 2, 3, 1).reshape(pred_reg.shape[0], -1, 4))
            all_refined_pred_reg.append(refined_pred_reg.permute(0, 2, 3, 1).reshape(pred_reg.shape[0], -1, 4))
        all_pred_reg = torch.cat(all_pred_reg, axis=1).reshape(-1, 4)
        all_refined_pred_reg = torch.cat(all_refined_pred_reg, axis=1).reshape(-1, 4)
        
        pred_reg_list = [
            _.permute(0, 2, 3, 1).reshape(pred_reg[0].shape[0], -1, 8)
            for _ in pred_reg_list]
        pred_regs = torch.cat(pred_reg_list, axis=1).reshape(-1, 8)
        all_pred_mean = pred_regs[:, :config.num_cell_anchors * 4]
        all_pred_lstd = pred_regs[:, config.num_cell_anchors * 4:]
        # get ground truth
        loss_dict = freeanchor_loss(all_anchors, all_pred_cls, all_pred_reg, gt_boxes, im_info)
        refined_loss_dict = freeanchor_loss(all_anchors, all_pred_cls, all_refined_pred_reg, gt_boxes, im_info)
        loss_kld = kldiv_loss(
                all_pred_mean,
                all_pred_lstd,
                config.kl_weight)
...
class RetinaNet_Head(nn.Module):
    ...
    def forward(self, features):
        ...
        pred_cls_list = [
            ...
            for _ in pred_cls]
        pred_reg_list = pred_reg
        return pred_cls_list, pred_reg_list, self.refined_reg_pred 

def per_layer_inference(anchors_list, pred_cls_list, pred_reg_list, refined_net, im_info):
    keep_anchors = []
    keep_cls = []
    keep_reg = []
    keep_lstd = []
    ...
    for l_id in range(len(anchors_list)):
        ...
        pred_mean = pred_reg_list[l_id][:, :config.num_cell_anchors * 4]
        pred_lstd = pred_reg_list[l_id][:, config.num_cell_anchors * 4:]
        scale = torch.tensor(config.prior_std).type_as(pred_lstd)
        pred_scale_std = pred_lstd.exp().permute(0,2,3,1).mul(scale).permute(0,3,1,2)
        pred_box = pred_mean + pred_scale_std * torch.randn_like(pred_lstd)
        # refined regression
        in_refined_reg = torch.cat([pred_box, pred_lstd], dim=1)
        refined_pred_reg = refined_net(in_refined_reg)
        pred_reg = refined_pred_reg.permute(0,2,3,1).reshape(refined_pred_reg.shape[0],-1,4)
        pred_lstd = pred_lstd.permute(0,2,3,1).reshape(pred_lstd.shape[0],-1,4)
