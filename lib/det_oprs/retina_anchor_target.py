...
@torch.no_grad()
def retina_avpd1_anchor_target(avpd_anchors, anchors, gt_boxes, im_info, top_k=1):
...
@torch.no_grad()
def retina_avpd2_anchor_target(avpd_anchors, anchors, gt_boxes, im_info, top_k=1):
    total_anchor = avpd_anchors.shape[0]
    return_labels = []
    return_bbox_targets = []
    avpd_anchors = avpd_anchors.reshape(config.train_batch_per_gpu, -1, 4)
