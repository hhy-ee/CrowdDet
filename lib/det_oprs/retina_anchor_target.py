...
@torch.no_grad()
def retina_anchor_target_m1vpd(anchors, gt_boxes, im_info, top_k=1):
    total_anchor = anchors.shape[0]
    return_labels = []
    return_bbox_targets = []
    ...
    for bid in range(config.train_batch_per_gpu):
        gt_boxes_perimg = gt_boxes[bid, :int(im_info[bid, 5]), :]
        anchors = anchors.type_as(gt_boxes_perimg)
        giou, overlaps = box_giou_opr(anchors, gt_boxes_perimg[:, :-1])
        # gt max and indices
        max_overlaps, gt_assignment = overlaps.topk(top_k, dim=1, sorted=True)
        max_overlaps= max_overlaps.flatten()
        gt_assignment= gt_assignment.flatten()
        _, gt_assignment_for_gt = torch.max(overlaps, axis=0)
        # overlap max and indices
        _, giou_indices = giou.sort(descending=True, dim=1)
        overlap_gt_assignment = giou_indices[:, 1]
        del giou, overlaps
        # cons labels
        labels = gt_boxes_perimg[gt_assignment, 4]
        labels = labels * (max_overlaps >= config.negative_thresh)
        ...
        target_anchors = anchors.repeat(1, top_k).reshape(-1, anchors.shape[-1])
        bbox_targets = bbox_transform_opr(target_anchors, target_boxes)
        # cons overlap bbox targets
        target_overlap_boxes = gt_boxes_perimg[overlap_gt_assignment, :4]
        target_anchors = anchors.repeat(1, top_k).reshape(-1, anchors.shape[-1])
        overlap_bbox_targets = bbox_transform_opr(target_anchors, target_overlap_boxes)
    ...
    if config.train_batch_per_gpu == 1:
        return labels, bbox_targets, gt_assignment
    else:
        return_labels = torch.cat(return_labels, axis=0)
        return_bbox_targets = torch.cat(return_bbox_targets, axis=0)
        return_overlap_bbox_targets = torch.cat(return_overlap_bbox_targets, axis=0)
        return return_labels, return_bbox_targets, return_overlap_bbox_targets

@torch.no_grad()
def retina_anchor_target_mmdet(anchors, gt_boxes, im_info, top_k=1):
    total_anchor = anchors.shape[0]
    return_labels = []
    return_bbox_targets = []
    # get per image proposals and gt_boxes
    for bid in range(config.train_batch_per_gpu):
        ...
        not_ignored_gt_indices = torch.where(gt_boxes_perimg[:,-1] == 1)[0]
        gt_boxes_perimg = gt_boxes_perimg[not_ignored_gt_indices, :]
        ...
        del overlaps
        # cons labels
        labels = gt_assignment.new_full((len(gt_assignment), ), -1, dtype=torch.float32)
        labels[max_overlaps < config.negative_thresh] = 0
        labels[max_overlaps >= config.positive_thresh] = 1
